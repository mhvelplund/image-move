...
def run():
  ...
  if not source_path.exists():
    print(f'Source folder "{source_path}" does not exist.', file=sys.stderr)
    exit(1)

  if not source_path.is_dir():
    print(f'Source "{source_path}" is not a folder.', file=sys.stderr)
    exit(1)

  destination_path = Path(args.destination).resolve()

  if not destination_path.exists():
    print(f'Destination folder "{destination_path}" does not exist.', file=sys.stderr)
    exit(1)

  if not destination_path.is_dir():
    print(f'Destination "{destination_path}" is not a folder.', file=sys.stderr)
    exit(1)

  prefix = args.prefix or source_path.name

  move_images(source_path, destination_path, args.copy, prefix, args.verbose, args.dry_run)
